<script lang="ts">
    ...
    const PUBLIC_HOME_URL = 'https://one-in-emilien.com/';
...
<a
    {href}
    class="{href ? '' : 'pointer-events-none'} {circled
        ? 'flex h-16 w-16 items-center justify-center rounded-full border-2 border-current p-4'
        : ''} "
    target="_blank"
>
    <slot />
    ...
    {:else}
        <div bind:this={container} in:fade></div>
    {/if}
</a>
