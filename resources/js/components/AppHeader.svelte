<script lang="ts">
	...
	const auth = $derived($page.props.auth as { user: User });

	const isCurrentRoute = $derived.by(() => (url: string) => urlIsActive(url, $page.url));

	const activeItemStyles = $derived.by(
		() => (url: string) =>
			...
			<div class="lg:hidden">
				<Sheet>
					<SheetTrigger asChild let:builder>
						<Button builders={[builder]} variant="ghost" size="icon" class="mr-2 h-9 w-9">
							<Menu class="h-5 w-5" />
						</Button>
					</SheetTrigger>
